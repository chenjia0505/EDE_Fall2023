...
EPA_summary<-data.frame(
  Item=c("Source", "Date", "Fiilename"),
  Value=c("EPA Air Quality System (AQS)", "2018-2019",
  "EPAair_O3_PM25_NC1819_Processed.csv")
)
knitr::kable(EPA_summary,caption = "Summary of EPA Air Quality data")
```
...
```{r fig.height=3.5, fig.align='left', fig.cap= "Monthly Ozone, 2018",echo=FALSE, message=FALSE}
...
epa_data %>% 
  ...
  ggplot(
    aes(
      ...
      y=Ozone))+
  geom_boxplot()+
  labs(y= expression(Concentration ~ (mu*g / m^3), x = "Month"))


```

```{r fig.height=3.5, fig.align='left', fig.cap= "Monthly Ozone, 2019",echo=FALSE, message=FALSE}
epa_data %>% 
  filter(Year==2019) %>% 
  drop_na(Ozone) %>% 
  ggplot(
...
## Git Repository
https://github.com/chenjia0505/EDE_Fall2023/tree/39dad46431f8053892f8fef72b5218931751d754/Assignments